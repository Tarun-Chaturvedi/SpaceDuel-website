...
pygame.display.set_caption("TGP1 !")
...
RED_SPACESHIP_IMAGE = pygame.image.load(os.path.join('Assets','spaceship_re.png'))
...
def blue_handle_movement(keys_pressed,blue):
    if keys_pressed[pygame.K_a] and blue.x - VEL > 0: # LEFT
        blue.x-= VEL
    if keys_pressed[pygame.K_d] and blue.x + VEL < BORDER.x - 35: # RIGHT
        blue.x+= VEL
    if keys_pressed[pygame.K_w] and blue.y - VEL > 0: # UP
        blue.y-= VEL
    if keys_pressed[pygame.K_s] and blue.y + VEL < HEIGHT - 50: # DOWN
        blue.y+= VEL
